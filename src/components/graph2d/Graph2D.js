...
const drawFunctionData = (graph, f) => {
  try {
    ...
    if (f.tangentPoint !== '') {
      graph.drawTangentWithDerivative(func, Number(f.tangentPoint));
    }
    if (f.zeroRange?.[0] !== '' && f.zeroRange?.[1] !== '') {
      graph.drawZeroPoints(func, Number(f.zeroRange[0]), Number(f.zeroRange[1]));
    }
  } catch {}
};

const drawFPS = (canvasApi, fps) => {
  if (!canvasApi || !canvasApi.text) return;
  const win = canvasApi.WIN;
  canvasApi.text(`FPS: ${fps}`, win.LEFT, win.BOTTOM + win.HEIGHT - 5, "red");
};
...
const Graph2D = () => {
  ...
  const handleAdd = () => {
    setFunctions(f => [...f, { funcStr: '', color: '#ff0000' }]);
  };

  const handleUpdate = (i, data) => {
    setFunctions(f => f.map((el, idx) => idx === i ? { ...el, ...data } : el));
  };
